...
void producer_file_reader(std::vector<std::pair<std::string, int>>::const_iterator start, 
    ...
    // Turn reader loop into an infinite industrial recycling pipeline
    while (g_server_running.load()) {
        auto t_io_start = std::chrono::high_resolution_clock::now();
        cv::Mat frame = cv::imread(current_it->first); 
        ...
        if (!frame.empty()) { 
            ...
            FrameData data; 
            data.frame = frame; 
            data.filename = fs::path(current_it->first).filename().string(); 
            data.label = current_it->second; 
            data.io_read_ms = io_ms;
             
            ring_buffer.push(std::move(data)); 
        }

        // Increment pointer to next file element
        current_it++;

        // RECYCLING BLOCK: If this thread reaches the end of its sliced subset, reset back to start
        if (current_it == end) {
            current_it = start; 
            
            // Asynchronously preserve subset chunk processing metrics to global master storage without stalling
    ...
    // Flush any remaining trailing metrics out before worker dissolution
    std::lock_guard<std::mutex> lock(g_metrics_mutex);
    if (!local_io_lats.empty()) {
        g_io_read_lats.insert(g_io_read_lats.end(), local_io_lats.begin(), local_io_lats.end());
    }
...
void opc_server_worker(std::string telemetry_log_path, std::string profile_str) {
    std::cout << "🌐 [OPC-NETWORK] Initializing native host server architecture..." << std::endl;
    
    // 1. Safe instance instantiation
    UA_Server *server = UA_Server_new();
    UA_ServerConfig *config = UA_Server_getConfig(server);

    // 2. Load standard baseline profiles and configurations cleanly
...
int main() { 
    ...
    std::thread opc_thread(opc_server_worker, telemetry_log_path, profile_str);
    ...
    while (ring_buffer.pop(job)) { 
        // Pause pipeline processing until an external PLC client requests an inspection
        while (!g_opc_trigger.load() && g_server_running.load()) {
            std::this_thread::sleep_for(std::chrono::microseconds(200)); // Yields CPU to prevent core thrashing
        }
    ...
    for (int final_slot = 0; final_slot < 2; ++final_slot) {
        if (!first_iterations[final_slot]) {
            ...
            // --- ZONE 4.4: MODEL PREDICTION HANDSHAKE RELEASES (DRAIN LOOP) ---
            int dynamic_status_code = predicted_anomaly ? 1 : 0; // 1 = FAIL, 0 = PASS
            g_opc_result.store(dynamic_status_code);
            g_opc_anomaly_score.store(static_cast<float>(max_val));
